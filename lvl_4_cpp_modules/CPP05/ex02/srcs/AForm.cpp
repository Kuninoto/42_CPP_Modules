...
AForm::AForm()
: name("Default"), grade_to_sign(150), grade_to_execute(150), is_signed(false) {
	cout << "[AForm] Constructor called" << endl;
}

AForm::AForm(std::string name, std::string target, int grade_to_sign, int grade_to_execute)
: name(name), target(target), grade_to_sign(grade_to_sign), grade_to_execute(grade_to_execute), is_signed(false)
{
	cout << "[AForm] Parametrized constructor called" << endl;
...
: name(to_copy.getName()), grade_to_sign(to_copy.getGradeToSign()), grade_to_execute(to_copy.getGradeToExecute()), is_signed(to_copy.getSignState())
{
